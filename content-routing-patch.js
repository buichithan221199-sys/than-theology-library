...
function uploadRoutingModal(mode){
  ...
  d.querySelector('#process').onclick=async()=>{
    ...
    try{
      ...
      const r=await fetch(AI,{method:'POST',headers:{},body:fd});const j=await r.json();if(!r.ok)throw Error(j.error||'Không xử lý được');d.remove();
