...
(function(){
  ...
  // Shortcut from a lesson detail page: choose image or PDF, then preselect
  // "Đáp án trắc nghiệm" and the current lesson automatically.
  window.openAnswerKeyUpload=function(lessonId){
    const lesson=S.lessons.find(x=>x.id===lessonId);if(!lesson)return;
    const d=overlay(`<div class="modal"><button class="x">×</button><h2>Tải đáp án trắc nghiệm</h2><p class="muted">Bài: <b>${esc(lesson.title||'')}</b></p><p>Chọn định dạng file đáp án.</p><div class="methods" style="grid-template-columns:repeat(2,1fr)"><button data-answer-mode="images">🖼️<br>Hình ảnh</button><button data-answer-mode="pdf">📄<br>PDF</button></div><p class="muted">Ảnh chỉ dùng tạm để AI đọc và sẽ không được lưu. PDF vẫn được giữ làm tài liệu nguồn.</p></div>`);
    d.querySelectorAll('[data-answer-mode]').forEach(b=>b.onclick=()=>{
      const mode=b.dataset.answerMode;d.remove();uploadRoutingModal(mode);
      const ov=[...document.querySelectorAll('.overlay')].at(-1);if(!ov)return;
      const kind=ov.querySelector('#contentKind'),target=ov.querySelector('#lessonTarget');
      if(kind){kind.value='answer_key';kind.dispatchEvent(new Event('change',{bubbles:true}))}
