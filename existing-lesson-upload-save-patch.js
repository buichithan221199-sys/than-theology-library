...
(function(){
  ...
  window.mediaSourcePreview=function(v,ctx){
    ...
    const fidelity=v?.source_notes==='fidelity_import_v1';
    const d=overlay(`<div class="modal" style="width:min(920px,96vw)"><button class="x">×</button><h2>Xem trước bài học</h2><p class="muted">${fidelity?`Bài học thần học sẽ giữ <b>100% nội dung nguồn đã đọc</b> trong thân bài, theo đúng thứ tự. AI chỉ đặt tiêu đề các đoạn và tạo Tóm tắt + Bài học rút ra.`:`Nội dung sẽ cập nhật trực tiếp vào: <b>${esc(lesson.title||'')}</b>.`} Bộ trắc nghiệm hiện có được giữ nguyên.</p><div class="field"><label>Tiêu đề nguồn</label><input id="mt" value="${esc(v?.title||'Tài liệu bài học')}"></div><div class="field"><label>Tóm tắt</label><textarea id="ms" rows="5">${esc(v?.summary||'')}</textarea></div><div class="muted" style="margin:8px 0 14px">${pointCount?`${fidelity?'Nguồn được chia':'AI đã tạo'} ${pointCount} phần để dễ đọc. Nội dung thân bài không bị cắt giảm.`:'Chưa có thân bài.'}</div><div class="field"><label>Nội dung nguồn đã đọc</label><textarea id="mx" rows="12">${esc(transcript)}</textarea></div><div id="saveLessonUploadErr"></div><div class="modalActions"><button class="btn gold" id="save">Cập nhật bài học + lưu file nguồn</button></div></div>`);
    ...
    d.querySelector('#save').onclick=async()=>{
      ...
      try{
        const updated={
          ...
          introduction:fidelity?'':pickText(lesson.introduction,v?.introduction),
          background:fidelity?'':pickText(lesson.background,v?.background),
          transition_text:fidelity?'':pickText(lesson.transition_text,v?.transition_text),
          main_content:fidelity?(Array.isArray(v?.main_content)?v.main_content:[]):pickArray(lesson.main_content,v?.main_content),
          application:fidelity?String(v?.application||''):pickText(lesson.application,v?.application),
          reflection_questions:fidelity?[]:pickArray(lesson.reflection_questions,v?.reflection_questions),
          prayer:fidelity?'':pickText(lesson.prayer,v?.prayer),
          ...
          source_notes:fidelity?'fidelity_import_v1':(lesson.source_notes||v?.source_notes||''),
          is_published:lesson.is_published!==false
        };

        // Save visible lesson content first. Quiz rows are stored separately and remain untouched.
        ...
        try{
          const text=d.querySelector('#mx').value;
        ...
        else alert(fidelity?'Đã cập nhật bài học với toàn bộ nội dung nguồn và lưu file nguồn.':'Đã cập nhật nội dung bài học và lưu file nguồn.');
      }catch(e){
