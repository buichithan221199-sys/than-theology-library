...
(function(){
  ...
  function previewEphemeralExistingLesson(v,ctx){
    ...
    const d=overlay(`<div class="modal" style="width:min(980px,96vw)"><button class="x">×</button><h2>Xem trước bài học đầy đủ</h2><p class="muted">File ${ctx.mode==='audio'?'audio':'hình ảnh'} sẽ <b>không được lưu</b>. Với audio, app giữ <b>toàn bộ nội dung nguyên bản có giá trị</b>, chỉ sắp xếp lại theo đúng mạch bài cho dễ đọc. Không dùng khuôn bài học Kinh Thánh. Phần cuối chỉ có <b>Tóm tắt các ý chính → Bài học rút ra</b>.</p><div class="field"><label>Tựa đề</label><input id="epTitle" value="${esc(valueOr(old.title,v?.title||''))}"></div><div style="margin:14px 0"><div style="font-weight:900;color:#15365e;font-size:19px;margin-bottom:8px">Nội dung bài học chi tiết</div><div style="max-height:52vh;overflow:auto;padding:4px 8px 4px 0">${detailed}</div></div><div class="field"><label>Tóm tắt các ý chính · phía sau bài học</label><textarea id="epSummary" rows="7">${esc(valueOr(old.summary,v?.summary||''))}</textarea></div><div class="field"><label>Bài học rút ra</label><textarea id="epApplication" rows="6">${esc(valueOr(old.application,v?.application||''))}</textarea></div><div id="epErr"></div><div class="modalActions"><button class="btn gold" id="epSave">Cập nhật toàn bộ bài học</button></div></div>`);
    d.querySelector('#epSave').onclick=async()=>{
      const b=d.querySelector('#epSave');b.disabled=true;b.textContent='Đang lưu…';
      try{
        const lesson={id:old.id,folder_id:old.folder_id,title:d.querySelector('#epTitle').value.trim()||old.title,subtitle:valueOr(old.subtitle,v?.subtitle||''),scripture_reference:valueOr(old.scripture_reference,v?.scripture_reference||''),key_verse_reference:valueOr(old.key_verse_reference,v?.key_verse_reference||''),key_verse_1925:valueOr(old.key_verse_1925,v?.key_verse_1925||''),teacher:valueOr(old.teacher,v?.teacher||''),lesson_date:old.lesson_date||null,summary:d.querySelector('#epSummary').value.trim(),introduction:ctx.mode==='audio'?'':valueOr(old.introduction,v?.introduction||''),background:ctx.mode==='audio'?'':valueOr(old.background,v?.background||''),transition_text:ctx.mode==='audio'?'':valueOr(old.transition_text,v?.transition_text||''),main_content:ctx.mode==='audio'?(Array.isArray(v?.main_content)?v.main_content:[]):arrayOr(old.main_content,v?.main_content),application:d.querySelector('#epApplication')?.value.trim()||valueOr(old.application,v?.application||''),reflection_questions:ctx.mode==='audio'?[]:arrayOr(old.reflection_questions,v?.reflection_questions),prayer:ctx.mode==='audio'?'':valueOr(old.prayer,v?.prayer||''),tags:arrayOr(old.tags,v?.tags),source_kind:old.source_kind||ctx.mode,is_published:old.is_published!==false};
