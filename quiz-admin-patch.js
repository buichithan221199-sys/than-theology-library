...
(function(){
  function selectedLetters(v){return [...new Set(String(v||'').toUpperCase().match(/[A-D]/g)||[])].sort()}
  ...
  window.editQuizAnswer=async function(questionId){
    ...
    const current=await getAnswerSafe(questionId);const checked=new Set(selectedLetters(current?.correct_option));const o=q.options||{};
    const d=overlay(`<div class="modal" style="width:min(760px,96vw)"><button class="x">×</button><h2>Chỉnh sửa đáp án</h2><p style="font-weight:800;line-height:1.55">${esc(q.question_text||'')}</p><div class="opts">${['A','B','C','D'].map(k=>o[k]?`<label class="opt" style="display:flex;gap:10px;align-items:flex-start;cursor:pointer"><input type="checkbox" data-answer="${k}" ${checked.has(k)?'checked':''} style="width:auto;margin-top:4px"><span><b>${k}.</b> ${esc(o[k])}</span></label>`:'').join('')}</div><div class="field"><label>Lời giải / ghi chú</label><textarea id="manualExplanation" rows="5" placeholder="Có thể để trống nếu chưa cần lời giải.">${esc(current?.explanation||'')}</textarea></div><div id="manualErr"></div><div class="modalActions"><button class="btn gold" id="manualSave">Lưu đáp án</button></div></div>`);
  ...
  window.showAllQuizAnswers=async function(){
    ...
    const missing=rows.filter(x=>!selectedLetters(x.a?.correct_option).length).length;d.querySelector('.muted').textContent=`${rows.length} câu · ${missing} câu chưa có đáp án. Một câu có thể có 1–4 đáp án đúng.`;
